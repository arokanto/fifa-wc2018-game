...
function getGuessesForClosedMatches(data, userData) {
  let players = []
  for (let i = 0; i < userData.length; i++) {
    let thisPlayer = userData[i]
    let returnPlayer = {}
    returnPlayer.display_name = thisPlayer.display_name
    let finishedMatches = {}
    for (let key in thisPlayer.group_matches) {
      let matchId = parseInt(key.split('_')[1])
      let matchData = findMatchById(data, matchId)
      if (matchData && matchData.finished) {
        let thisMatch = thisPlayer.group_matches[key]
        if (matchData.home_result == thisMatch.home && matchData.away_result == thisMatch.away) {
          thisMatch.correct = true
        }
        finishedMatches['match_' + matchId] = thisMatch
      }
    }
    returnPlayer.guesses = finishedMatches
